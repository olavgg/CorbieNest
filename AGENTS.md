...
`THINKTAG`, `EVICTMAIN` from the brief. Its models differ in what they can do: `fake-thinker` is on/off,
`fake-levels` is gpt-oss-like, `fake-declared` lists its levels in `/api/show`, `fake-stale` is
under-reported by `/api/tags`, `fake-remote:cloud` exists for `/api/show` only and answers 401). Run the tests after every change to `src/`; both suites must pass and the
...
- Anything that waits (HTTP streaming, shell commands) must poll `term_poll_interrupt()`: it is
  ...
  consumed by `main.c` — between tool rounds (`inject_queued()`, which takes the plain messages
  and steps over any queued `/`/`!` line, since that one is the REPL's) and after the turn
  (`process_input` loop); after an interrupt they go back to the editor
  (`term_queue_to_editor()`), never silently dropped. The prompt's idle work is busy in the same
  way (the memory extraction is a model call of its own) but is *not* inside that REPL loop, so
  `readline_impl` submits the oldest queued line itself once `ed_idle_run()` returns — otherwise
  a message queued while memory was being written would sit there until the user pressed Enter
  a second time. Anything else that runs work at the prompt has to do the same.
- A queued message is *urgent*: it is nearly always about the job in flight, so work stops for
  it rather than the other way round. `term_queue_mark()` / `term_queue_new()` are the test —
  a plain message queued *since the mark* stops the shell command that is running (`run_shell`
  returns 125, `t_bash` tells the model the command was cut short), makes `run_turn` skip the
  calls of the round that have not started (each still gets a `TOOL_NOT_RUN` result: the
  conversation must stay well-formed) and makes `run_subagent` report what it has instead of
  finishing. `run_turn` marks at the start and `inject_queued()` after each drain, so messages
  that were already waiting when the turn began are delivered normally and kill nothing. Any new
  long-running wait should follow the same rule.
...
- Tool confirmations obey `g_cfg.mode` (`MODE_MANUAL`, `MODE_ACCEPT_EDITS`, `MODE_PLAN`,
  `MODE_AUTO`); route new mutating tools through `confirm(what, CONF_EDIT|CONF_BASH|CONF_FETCH, ...)`
  in `tools.c`. A new kind needs its own `perm_match()` case, its rule text (`bash <words>`,
...
- The advisor is a tool like `task`, but its failures must never end the turn: every outcome of
  ...
  (`ADVISOR_MAX`). Its prompt tokens go through `account()` only, never into
  `last_prompt_tokens`/`ctx_measured()`: that is another model's prompt, and counting it would
  make auto-compact fire on a conversation that is nowhere near full.
