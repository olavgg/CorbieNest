...
canned response. Its models differ in what they can do: `fake-thinker` is on/off, `fake-levels` is
gpt-oss-like, `fake-declared` lists its levels in `/api/show`, `fake-stale` is under-reported by
`/api/tags`). Run the tests after every change to `src/`; both suites must pass and the
...
- Capabilities and effort levels come from `/api/show` (`g_model_info`), not `/api/tags`: the
  list reports a model as it was pulled.
