...
| `src/tools.c` | tool definitions + implementations, `confirm()` (mode-aware; 4th menu option saves a rule to `.corbienest/permissions`, `tools_permissions_*`), checkpoints for `/rewind` (`tools_checkpoint_*`: pre-write file states per request), shell runner |
