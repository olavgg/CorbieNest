...
check(s.expect("memory: no change ·"), f"memory step timing shown: {s.text()[-200:]!r}")
...
s = Session(["-m", "fake-coder:latest", "--yolo"]); s.expect("›")
...
s.send("\x1b[5~"); s.expect("scrollback"); check("a-2" in since(), f"PgUp again scrolls further up: {since()[-300:]!r}")
